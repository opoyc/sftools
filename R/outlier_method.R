#' Performs outlier method to Kinaxis data
#'
...
outlier_method <- function(time_series, outlier_method, data_rule, mov_avg_n, threshold, causal_factor = T) {
  if (outlier_method == "Standard Deviation") {
    if (data_rule == "Historical") { # same as kinaxis
      ...
      avg <- mean(new_ts)
      std_d <- sd(new_ts)
      new_ts[new_ts < avg - threshold * std_d] <- avg - threshold * std_d # lower threshold
      new_ts[new_ts > avg + threshold * std_d] <- avg + threshold * std_d # upper threshold

      if (sum(new_ts) == 0) {
        new_ts <- time_series
      } # if outlie method changes all values to zero, revert back to original ts
      ...
      return(list(
        ...
        upper_threshold = replace(x = time_series, list = 1:length(time_series), values = avg + threshold * std_d),
        lower_threshold = replace(x = time_series, list = 1:length(time_series), values = avg - threshold * std_d)
      ))
    }
    if (data_rule == "Moving Average Error") { # ok
      ...
      std_d <- sd(resi)
      resi_new <- resi
      resi_new[resi_new < (avg - threshold * sd(resi)) ] <- (avg - threshold * sd(resi)) # lower threshold
      resi_new[resi_new > (avg + threshold * sd(resi)) ] <- (avg + threshold * sd(resi)) # upper threshold
      ...
      return(list(
        ...
        upper_threshold = (time_series - resi + threshold * std_d),
        lower_threshold = (time_series - resi - threshold * std_d)
      ))
    }
    if (data_rule == "Rstl Error" & causal_factor) { # ok

      rstl <- stlplus(time_series, s.window = "periodic", n.p = 12)
      resi <- rstl$data[, "remainder"]

      avg <- mean(resi[!(is.na(resi))])
      std_d <- sd(resi[!(is.na(resi))])
      resi_new <- resi
      resi_new[resi_new < (avg - threshold * sd(resi)) ] <- (avg - threshold * std_d) # lower threshold
      resi_new[resi_new > (avg + threshold * sd(resi)) ] <- (avg + threshold * std_d) # upper threshold
      ...
      return(list(
        ...
        upper_threshold = (time_series - rstl$data[, "remainder"] + threshold * std_d),
        lower_threshold = (time_series - rstl$data[, "remainder"] - threshold * std_d)
      ))
    }
    if (data_rule == "Rstl Error" & !causal_factor) { # use stl for ts with causal factor
      ...
      return(list(
        ...
        method = "Standard Deviation",
        data_rule = "Rstl Error",
        upper_threshold = (time_series - rstl$time_series[, 3] + threshold * std_d),
        lower_threshold = (time_series - rstl$time_series[, 3] - threshold * std_d)
      ))
    }
  }
  ...
  if (outlier_method == "Iglewicz Hoaglin Method") {
    if (data_rule == "Historical") { # ok
      dist_lower <- median(time_series) - threshold * (mad(time_series, constant = 1) / 0.6745)
      dist_upper <- median(time_series) + threshold * (mad(time_series, constant = 1) / 0.6745)
    ...
    if (data_rule == "Moving Average Error") { # ok
      ...
      dist_lower <- -threshold * (mad(resi, constant = 1) / 0.6745)
      dist_upper <- threshold * (mad(resi, constant = 1) / 0.6745)
    ...
    if (data_rule == "Rstl Error" & causal_factor) { # ok
      ...
      dist_lower <- -threshold * (mad(resi[!is.na(resi)], constant = 1) / 0.6745)
      dist_upper <- threshold * (mad(resi[!is.na(resi)], constant = 1) / 0.6745)
  ...
  if (outlier_method == "Winsorizing") {
    if (data_rule == "Historical") { # same as Kinaxis
      ...
      new_ts <- time_series
      new_ts <- Winsorize(new_ts, probs = c(threshold, 1 - threshold), na.rm = TRUE)
    ...
    if (data_rule == "Moving Average Error") { # same as Kinaxis
      ...
      win_resi <- Winsorize(resi, probs = c(threshold, 1 - threshold))
    ...
    if (data_rule == "Rstl Error" & causal_factor) {
      # This is not correct. normal STL should be applied if there's no regressor attached to that GMID.
      ...
      if (class(stl_try) != "try-error") {
        ...
        resi_new <- resi
        win_resi <- Winsorize(resi, probs = c(threshold, 1 - threshold), na.rm = T)
  ...
  if (outlier_method == "IQR") {
    new_ts <- time_series
    qnt <- quantile(new_ts, probs = c(.25, .75))
    caps <- quantile(new_ts, probs = c(.05, .95))
    H <- 1.5 * IQR(new_ts)
  ...
  warning("No method applied, original time series is returned.")
