...
detect_seas_input <- function(data, gmid, version="p_value"){
    fit=lm(Value ~ factor(month), data=data)
    fit_sum=summary(fit)
    fit_coefs=as.numeric(sum((fit_sum$coefficients[,4])<0.05, na.rm=TRUE))
    fit_fstat=pf(fit_sum$fstatistic[1], fit_sum$fstatistic[2], fit_sum$fstatistic[3], lower.tail = FALSE)
    if(version=="p_value"){
        if(fit_coefs>1){
            return(gmid)
        }
    } else {
        if(fit_fstat < 0.05 & !is.nan(fit_fstat)) {
            return(TRUE)
            } else {
                return(FALSE)
                }
